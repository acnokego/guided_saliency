...
def autoencoder_batch_iterator_finetune(model, train_data):
    num_epochs = 10
    nr_batches_train = int(len(train_data) / model.batch_size)
    n_updates = 1
    ...
    for current_epoch in tqdm(range(fine_tune_epochs), ncols=20):
        ...
        for currChunk in chunks(train_data, model.batch_size):
            ...
            batch_input_imgs = np.asarray([style.prep_image(x.image.data).astype(theano.config.floatX).reshape(3,INPUT_SIZE[1],INPUT_SIZE[0]) 
                                           for x in currChunk], dtype=theano.config.floatX)
...
def train():
    ...
    with open('/home/yuandy/COCO_dataset/processed_data/320x240/trainData.pickle', 'rb') as f:
    ...
    autoencoder_batch_iterator_finetune(model, train_data)
    '''
    if flag == 'salgan':
